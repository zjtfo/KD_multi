...
#     parser.add_argument("--teacher-ckpt-kopl", default='./Bart_Program_50/output/checkpoint/checkpoint-best')
...
# checkpoint方式采用github上的(权重由0.8依次改为0.7、0.4)
import os
import torch
import torch.optim as optim
import torch.nn as nn
...
def train(args):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_loader):
            ...
            loss = loss_ce.mean() + 0.4 * rep_loss + sparql_kd_weight * transfer_sparql_loss_ce.mean() + dcs_kd_weight * transfer_dcs_loss_ce.mean()
...
def main():
    ...
    # input and output
    parser.add_argument('--input_dir', default='./try_cycle_generation/multi_teacher/Stage_2_3_three_teacher_selfkopl_encoder_sparql_dcs/preprocessed_data')
    parser.add_argument('--output_dir', default='./try_cycle_generation/multi_teacher/Stage_2_3_three_teacher_selfkopl_encoder_sparql_dcs/output_raw_04/checkpoint')

    parser.add_argument('--save_dir', default='./try_cycle_generation/multi_teacher/Stage_2_3_three_teacher_selfkopl_encoder_sparql_dcs/log_raw_04', help='path to save checkpoints and logs')
    parser.add_argument('--model_name_or_path', default='./bart-base')
